...
function Constraints(
	...
	if population_constraint === nothing
        population_constraint = PopulationConstraint(0, Inf)
    end
	constraints = Vector{AbstractConstraint}()
	descriptions = Vector{String}()
...
function add_constraint!(
	...
	if constraint isa PopulationConstraint
		constraints.population_constraint = constraint
		return
	end

	@assert length(constraints.constraints) == length(constraints.descriptions)
...
# 	kwargs...
# )::Bool
# 	# Support both argument orders for convenience while migrating old code.
# 	try
# 		return satisfies_fn(partition, constraint; kwargs...)
# 	catch e
# 		if isa(e, MethodError)
# 			return satisfies_fn(constraint, partition; kwargs...)
# 		end
# 		rethrow(e)
# 	end
# end
